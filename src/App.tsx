...
const products: Record<CartEdition, { name: string; price: number }> = {
  paperback: { name: 'Paperback', price: 4.95 },
  hardcover: { name: 'Hardcover', price: 7.95 }
};
...
function calculateShipping(items: CartCheckoutItem[]) {
  const paperbackQuantity = items.find((item) => item.edition === 'paperback')?.quantity ?? 0;
  const hardcoverQuantity = items.find((item) => item.edition === 'hardcover')?.quantity ?? 0;
  if (paperbackQuantity + hardcoverQuantity === 0) return 0;

  // Conservative handling reserve: first book absorbs publisher freight and packing; additional books share the shipment.
  const firstBookRate = hardcoverQuantity > 0 ? 19.95 : 12.95;
  const additionalHardcovers = Math.max(0, hardcoverQuantity - 1) * 10.95;
  const paperbackRate = hardcoverQuantity > 0 ? paperbackQuantity * 6.95 : Math.max(0, paperbackQuantity - 1) * 6.95;
  return firstBookRate + additionalHardcovers + paperbackRate;
}
...
export default function App() {
  ...
  const cartShipping = calculateShipping(cartItems);
  ...
  return (
    <div className="site-shell">
      <div className="top-note">Direct preorders ship in approximately 4 weeks · Shipping & handling calculated in cart</div>
      ...
      <main id="top">
        ...
        <section className="hero section-pad" aria-labelledby="hero-title">
          <div className="hero-copy">
            ...
            <p className="shipping-line"><span className="shipping-dot" aria-hidden="true" /> Shipping & handling is shown in your cart before payment</p>
          </div>
        ...
        <section id="preorder" className="preorder-wrap section-pad" aria-labelledby="preorder-title">
          <div className="preorder-heading"><p className="eyebrow">Direct preorder</p><h2 id="preorder-title">Choose your edition.</h2><p>Add paperbacks, hardcovers, or both. Your cart shows the complete order and shipping total before you pay.</p></div>
          <div className="edition-grid">
            <article className="edition-card paperback-card">
              ...
              <div className="price-line"><strong>$4.95</strong><span>book price</span></div><p className="savings-note">Shipping & handling is calculated in your cart</p>
            ...
            <article className="edition-card hardcover-card">
              ...
              <div className="price-line"><strong>$7.95</strong><span>book price</span></div><p className="savings-note">Shipping & handling is calculated in your cart</p>
        ...
        <section className="why-direct section-pad" aria-labelledby="why-direct-title"><div><p className="eyebrow">A transparent preorder</p><h2 id="why-direct-title">A little patience. A personal route.</h2></div><div className="why-direct-copy"><p>The publisher supplies direct copies in cases of 12. Your preorder helps fill the next case, then the order total shown at checkout covers the selected books and delivery to your U.S. address.</p><p>After the case is received, each copy is prepared and shipped directly to you. Please allow approximately four weeks from your preorder.</p><a className="button button-wine" href="#preorder">Reserve my copy <Arrow /></a></div></section>
        ...
        <section id="faq" className="faq section-pad" aria-labelledby="faq-title"><p className="eyebrow">Before you order</p><h2 id="faq-title">Common questions.</h2><div className="faq-list">
          ...
          <details><summary>How is shipping handled?<span aria-hidden="true">+</span></summary><p>Shipping and handling are calculated in your cart based on the editions and quantity selected. Direct preorder checkout currently serves U.S. addresses only.</p></details>
      ...
      {cartPanel === 'checkout' && <div className="cart-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setCartPanel('closed'); }}>
        <aside className="cart-drawer" role="dialog" aria-modal="true" aria-labelledby="cart-title">
          ...
          {cartItems.length === 0 ? <div className="cart-empty"><p>Your cart is empty.</p><button type="button" className="button button-wine" onClick={() => { setCartPanel('closed'); document.getElementById('preorder')?.scrollIntoView({ behavior: 'smooth' }); }}>Choose an edition <Arrow /></button></div> : <>
            ...
            <div className="cart-summary"><div><span>Books ({cartQuantity})</span><strong>${cartSubtotal.toFixed(2)}</strong></div><div><span>Shipping & handling</span><strong>${cartShipping.toFixed(2)}</strong></div><div className="cart-total"><span>Total</span><strong>${cartTotal.toFixed(2)}</strong></div></div>
